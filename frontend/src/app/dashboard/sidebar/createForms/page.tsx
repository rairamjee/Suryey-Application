...
function CreateForm() {
  ...
  const handleSubmitForm = async () => {
    ...
    try {
      ...
      console.log(error);
      setError("Failed to submit forms.");
    }
  };
  ...
  return (
    <div className="flex p-8 bg-slate-100 min-h-screen w-full">
      <div className="w-full">
            ...
            forms.map((form, index) => (
              <div key={index} className="mb-8">
                <div className="p-4 mb-4 bg-slate-950 rounded shadow text-white">
                  ...
                  <Button className="mt-8 bg-white text-slate-950 hover:bg-slate-100" onClick={handleAddQuestion}>
                    Add Question
                  </Button>
                </div>
        ...
        {forms.length > 0 && (
          <Button onClick={handleSubmitForm} className="mt-8">
            Submit Form
          </Button>
        )}
        ...
        <Dialog open={isTypeSelectionOpen} onOpenChange={setIsTypeSelectionOpen}>
          <DialogContent>
            ...
            <div className="space-y-4">
              <Button onClick={() => handleQuestionTypeSelect("TrueFalse")} className="ml-4">
                True/False
              </Button>
              <Button onClick={() => handleQuestionTypeSelect("Input")} className="ml-4">
                Input
              </Button>
              <Button onClick={() => handleQuestionTypeSelect("MCQ")} className="ml-4">
                MCQ
              </Button>
            </div>
            <DialogFooter>
              <Button onClick={() => setIsTypeSelectionOpen(false)} variant="secondary">
                Cancel
              </Button>
            </DialogFooter>
          </DialogContent>
        ...
        <Dialog open={selectedQuestionType !== null} onOpenChange={() => setSelectedQuestionType(null)}>
          <DialogContent>
            ...
            <DialogFooter>
              <Button onClick={() => setSelectedQuestionType(null)} variant="secondary">
                Cancel
              </Button>
